...
def sin2(x, xmin=-1, xmax=1):
    #x = rescale(x, xmin, xmax, -2,2)

    return 1-math.cos(math.pi*(x-1))/(abs(x-1)+1)

def parab(x,xmin=-1,xmax=1):
    rescale(x,xmin,xmax,-2,2)
    return x*x
def w(x, xmin=-1, xmax=1):
    x=rescale(x, xmin, xmax, -2, 2)
    return tf.abs(2*tf.abs(x) - 1) - x
...
zv = w(np.stack([_xv, _yv], 1))
#print zv
zv=tf.reduce_mean(zv,axis=1)
#print (tf.shape(zv))
sess = tf.Session()
